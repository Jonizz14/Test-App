...
const CustomLoader = ({ message = 'Yuklanmoqda...', size = 'medium' }) => {
  const sizeClass = size === 'large' ? 'custom-loader-large' : size === 'small' ? 'custom-loader-small' : '';
  
  return (
    <div className={`custom-loader-container ${sizeClass}`}>
      ...
      {message && (
        <p className="custom-loader-message">{message}</p>
      )}
    </div>
  );
};
...
// Full screen version
export const CustomFullScreenLoader = ({ message = 'Yuklanmoqda...', size = 'medium' }) => {
  return (
    <div className="custom-fullscreen-loader">
      <CustomLoader message={message} size={size} />
    </div>
  );
};
